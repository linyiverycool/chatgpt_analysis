...
void Flash_Init(void)
{
    ...
    {
        shift_X = *(uint16_t*)(0x807f002);   //存入取出里面的值
        shift_Y = *(uint16_t*)(0x807f004);
    ...
    {
        shift_X = 40;  // 自动采摘时 X,Y轴摄像头偏移量;X:0~80可调;Y:0~100可调;
        shift_Y = 50;
        MOT_Spwm_MAX = 1600;  // 	机械手舵机伸缩最大间距;max;1500~1750可调;
        MOT_Spwm_MIN = 1400;
        Pick_time_M = 14;   // 	Z轴出去或回来最大延时;
...
void Flash_eepromIn(void)
{
    ...
    FLASH_ProgramHalfWord(0x807f002, shift_X);
    FLASH_ProgramHalfWord(0x807f004, shift_Y);
    FLASH_ProgramHalfWord(0x807f006, MOT_Spwm_MAX/256);
    FLASH_ProgramHalfWord(0x807f008, MOT_Spwm_MAX%256);
    FLASH_ProgramHalfWord(0x807f00a, MOT_Spwm_MIN/256);
    FLASH_ProgramHalfWord(0x807f00c, MOT_Spwm_MIN%256);
...
void Set4_Adjust(void)
{
    ...
    {
    case 9:        //选择要调整的数值
        if(Set_Wei>0)Set_Wei=Set_Wei-1;
        break;	// 向左;
    case 13:
        Set_Wei=(Set_Wei+1)%6;
        break;	// 向右;
    case 1:
        ...
        {
        case 1:
            if(MOT_Spwm_MAX<1750)	MOT_Spwm_MAX = MOT_Spwm_MAX + 10;
            break;
        case 2:
            if(MOT_Spwm_MIN<1500)	MOT_Spwm_MIN = MOT_Spwm_MIN + 10;
            break;
        case 3:
            Pick_time_M =  Pick_time_M + 1;
            break;
        case 4:
            if(shift_X<80)	shift_X = shift_X + 1;
            break;
        case 5:
            if(shift_Y<100)	shift_Y = shift_Y + 1;
            break;
        default:
            break;
        }
    ...
    case 5:
        ...
        {
        case 1:
            if(MOT_Spwm_MAX>1500)	MOT_Spwm_MAX = MOT_Spwm_MAX - 10;
            break;
        case 2:
            if(MOT_Spwm_MIN>1250)	MOT_Spwm_MIN = MOT_Spwm_MIN - 10;
            break;
        case 3:
            if(Pick_time_M>0)	Pick_time_M =  Pick_time_M - 1;
            break;
        case 4:
            if(shift_X>0)	shift_X = shift_X - 1;
            break;
        case 5:
            if(shift_Y>0)	shift_Y = shift_Y - 1;
            break;
        default:
            break;
        }
    ...
    if(press_flag1==1)// 独立按键1 直接退回主界面 不保存数据;
    {
        shift_X = *(uint16_t*)(0x807f002);
        shift_Y = *(uint16_t*)(0x807f004);
        MOT_Spwm_MAX = (*(uint16_t*)(0x807f006))*256 + (*(uint16_t*)(0x807f008));
        MOT_Spwm_MIN = (*(uint16_t*)(0x807f00a))*256 + (*(uint16_t*)(0x807f00c));
        Pick_time_M = *(uint16_t*)(0x807f00e);
    ...
    if(Apple_Pick!=0)															// 采摘中;
    {
        ...
        case 5:		// 正常05;
            ...
            if(MOT_Spwm<MOT_Spwm_MAX)	MOT_Spwm=MOT_Spwm+10;
            if(Pick_time1>30) {
                Pick_time1=0;    // 果实计数;Run_Num++;Alarm_Ctrl();
                Apple_Pick=6;
            }
            break;
        case 6:		// 结束采摘;归0;
