...
{		 		
	if(EXTI_GetITStatus(EXTI_Line13)==SET)	//是8线的中断
	{      
		OV7670_WRST=0;	//复位写指针		  		 
		OV7670_WRST=1;	
	...
	EXTI_ClearITPendingBit(EXTI_Line13);  //清除EXTI8线路挂起位						  
} 
//外部中断8初始化
...
{												  
  ...
  GPIO_EXTILineConfig(GPIO_PortSourceGPIOF,GPIO_PinSource13);//PA8对中断线8
